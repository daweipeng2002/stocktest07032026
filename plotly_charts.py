...
def build_revenue_figure(revenue: pd.DataFrame) -> go.Figure:
    """精簡版季度營收柱狀圖：自動換算成百萬/十億單位，不用格線塞滿整個畫面。"""
    revenue = revenue.dropna(subset=["Revenue"])
    ...
    fig = go.Figure(go.Bar(
        x=labels, y=scaled, marker_color="#4c9aff",
        text=[f"{v:.1f}" for v in scaled], textposition="outside",
    ))
    fig.update_layout(
        template="plotly_dark",
        paper_bgcolor="#0e1117",
        ...
        yaxis_title=f"營收（{unit}）" if unit else "營收",
    )
    fig.update_xaxes(showgrid=False)
    fig.update_yaxes(showgrid=True, gridcolor="rgba(255,255,255,0.1)", showticklabels=False)
    return fig
